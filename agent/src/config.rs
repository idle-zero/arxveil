...
mod tests {
    ...
    #[test]
    fn accepts_https_configuration() {
        let config = AgentConfig::from_values(
        ...
        assert_eq!(config.server_endpoint, "https://arxveil.example");
    }

    #[test]
    fn rejects_missing_endpoint() {
        let error = AgentConfig::from_values(None, Some("state".to_owned()))
    ...
    fn rejects_blank_required_values() {
        let endpoint_error =
            AgentConfig::from_values(Some("  ".to_owned()), Some("state".to_owned()))
        ...
        let state_error =
            AgentConfig::from_values(Some("http://server:9090".to_owned()), Some(" ".to_owned()))
    ...
    fn rejects_endpoint_without_http_or_https_scheme() {
        let error =
            AgentConfig::from_values(Some("server:9090".to_owned()), Some("state".to_owned()))
